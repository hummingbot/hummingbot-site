...
`Order levels` are predefined price points for buying or selling assets, crucial in both manual and algorithmic trading. They optimize pricing, enhance risk management by diversifying entry and exit points, and aid in executing strategic trading decisions like dollar-cost averaging or stop-loss orders. 

These levels are often determined based on market analysis and help in maintaining emotional discipline in trading. Additionally, in high-frequency trading, setting various order levels contributes to market liquidity. Overall, order levels are key to effective trading strategies, enabling better market entry/exit timing and informed trading choices.


## Manual Order Levels

Below is a sample code you can use to manually configure your order levels and set the parameters manually in v2 strategies. 
