...
| [Order Levels](./order-levels/#orderlevelamount) | `order_level_amount` | `How much do you want to increase or decrease the order size for each additional order?` | The size can either increase(if set to a value greater than zero) or decrease(if set to a value less than zero) for subsequent order levels after the first level. |
| [Order Levels](./order-levels/#orderlevelspread) | `order_level_spread` | `Enter the price increments (as percentage) for subsequent orders?` | The incremental spread increases for subsequent order levels after the first level. |
...
| [Hanging Orders](./hanging-orders/#hangingorderscancelpct) | `hanging_orders_cancel_pct` | `At what spread percentage (from mid price) will hanging orders be canceled?` | Cancels the hanging orders when their spread goes above this value. |
...
| [Price Band](./price-band/#priceceiling) | `price_ceiling` | `Enter the price point above which only sell orders will be placed` | Place only sell orders when mid price goes above this price. |
| [Price Band](./price-band/#pricefloor) | `price_floor` | `Enter the price below which only buy orders will be placed` | Place only buy orders when mid price falls below this price. |
