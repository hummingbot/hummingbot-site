...
helper class designed to assist strategies with managing [hanging orders](hanging-orders.md). It automates
...
[extreme spreads](hanging-orders.md#hanging_orders_cancel_pct); second, it renews orders that have passed
the max order age. To enable renewing old orders, the strategy must implement the 
[`max_order_age`](max-order-age.md) attribute.
