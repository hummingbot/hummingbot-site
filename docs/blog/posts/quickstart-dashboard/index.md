...
- **[Miniconda or Anaconda](https://docs.conda.io/en/latest/miniconda.html)** — `conda` must be available on your `PATH`
- **[Hummingbot API](../../../hummingbot-api/quickstart.md)** — the Dashboard requires the API backend and MQTT broker to be running separately

## Installation

Clone the [Hummingbot Dashboard repository](https://github.com/hummingbot/dashboard), which contains the Streamlit app and Makefile for source installation.
...
`make install` creates a conda environment named `dashboard` and installs dependencies. `make run` starts the Dashboard Streamlit app.

After the app has started, access the Dashboard at <http://localhost:8501> in your browser.
...
Authentication is disabled by default. To enable Dashboard authentication, follow the steps below.

**Set credentials (optional):**

The dashboard uses `admin` and `abc` as the default username and password. Change these credentials for better security:

- Navigate to the `dashboard` folder and open the `credentials.yml` file.
- Add or modify the username and password, then save the file.
...
- Open the `CONFIG.py` file in the dashboard root folder.
- Locate the line `AUTH_SYSTEM_ENABLED = os.getenv("AUTH_SYSTEM_ENABLED", "False").lower() in ("true", "1", "t")`.
  ...
  ```
- Change the default value from `"False"` to `"True"` to enable dashboard authentication.
- Save `CONFIG.py`.
- Relaunch the dashboard by running `make run`.
