...
The **Agent Builders Cup** live trading finals are **October 1–2, 2026**, with winners announced October 7 at our Token2049 side event in Singapore. Sponsor teams: **[Orca](../exchanges/gateway/orca.md), [Derive](../exchanges/derive/index.md), [Gate](../exchanges/gate-io/index.md), [XRPL](../exchanges/xrpl.md), [Bitget](../exchanges/bitget/index.md), and [Meteora](../exchanges/gateway/meteora.md)**. See [botcamp.xyz/hackathons](https://www.botcamp.xyz/hackathons) for the schedule and livestream.

## 🐦 What's New - Hummingbot Client

### 🔌 New Connector (Kalshi Perpetual)

* **[Kalshi](https://kalshi.com)** – Perpetual connector for the USD-margined perpetual futures on Kalshi, the CFTC-regulated US exchange — its margin markets, not its event contracts (`KXBTCPERP` → `BTC-USD`). Full derivative lifecycle: RSA-PSS REST and WebSocket auth, order book and user streams, balances, positions, leverage, and funding. Resting close orders emulate `reduce_only` (Kalshi only accepts it on IOC) so a take-profit cannot flip you into the opposite position after the original exposure is gone. Also fixes shared `ExchangePyBase` order tracking so DNS/HTTP timeouts no longer count toward the lost-order limit.
...
**PRs:** [#8431](https://github.com/hummingbot/hummingbot/pull/8431) | [#8441](https://github.com/hummingbot/hummingbot/pull/8441) | [#8438](https://github.com/hummingbot/hummingbot/pull/8438) | **Thanks to [mlguys](https://github.com/mlguys)! 🙏**
...
Gateway adds **Meteora DAMM v2** (constant-product `cp-amm`) under `/trading/amm/*` as a second Meteora trading type alongside existing DLMM (`clmm`). Pool info, swap quote/execute, add/remove liquidity, and `create-pool` (requires an explicit `configAddress`; seed price defaults to the unified swap-router quote so new pools open on-market). Positions are NFTs — add/remove operate on the wallet's largest position unless a `positionAddress` is passed.

Meteora DLMM `openPosition` no longer rejects ranges wider than 69 bins. 69 is a **transaction** limit, not a position limit (`POSITION_MAX_LENGTH` is 1400). Wide ranges are chunked across transactions; `quote-liquidity` reports `positionCount` and `transactionCount` so callers see the cost before opening.
