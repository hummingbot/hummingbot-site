...
Following our [Technical Roadmap](https://hummingbot.org/blog/posts/hummingbot-2023-technical-roadmap/), we are expanding our connectors to support to include all order types offered by an exchange. In this release, we integrated the `market_order` type for [Ascendex](../exchanges/ascendex/index.md).
...
The following connectors and strategies did not receive enough votes in the Epoch 5 polls to meet the Minimum Voting Power threshold, so they have been removed from the Hummingbot codebase. Check the poll results here: https://hummingbot.org/blog/posts/epoch-5-polls-recap/
