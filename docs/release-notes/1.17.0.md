...
This release refactors the [Position Executor](/scripts/smart-components/#position-executor) smart component that creates self-executing [Triple Barrier](https://www.mlfinlab.com/en/latest/labeling/tb_meta_labeling.html) positions. Notably, this refactor integrates **both spot and perpetual support**, so that users can use the same component across both exchange types interchangeably.
