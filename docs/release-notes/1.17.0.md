...
This release refactors the Position Executor smart component that creates self-executing [Triple Barrier](https://web.archive.org/web/20240222013516/https://www.mlfinlab.com/en/latest/labeling/tb_meta_labeling.html) positions. Notably, this refactor integrates **both spot and perpetual support**, so that users can use the same component across both exchange types interchangeably.
...
For more information, see [this blog post](https://hummingbot.org/blog/posts/kicking-off-the-hummingbot-dashboard-community-project/) where we kicked off the Hummingbot Dashboard project and following progress in the **#dashboard** channel in Discord.
...
The following are connectors and strategies were removed from the Hummingbot codebase that didnt pass quorum in the previous Epoch 4 polls. Check the poll results here: https://hummingbot.org/blog/posts/epoch-4-polls-recap/
