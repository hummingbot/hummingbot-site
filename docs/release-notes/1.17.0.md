...
This release refactors the Position Executor smart component that creates self-executing [Triple Barrier](../strategies/v2-strategies/executors/positionexecutor.md#the-triple-barrier-method) positions. Notably, this refactor integrates **both spot and perpetual support**, so that users can use the same component across both exchange types interchangeably.
