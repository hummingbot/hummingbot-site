...
This release refactors the [Position Executor](../v2-strategies/candles/index.md) smart component that creates self-executing [Triple Barrier](https://www.mlfinlab.com/en/latest/labeling/tb_meta_labeling.html) positions. Notably, this refactor integrates **both spot and perpetual support**, so that users can use the same component across both exchange types interchangeably.
...
For more information, see [this blog post](../blog/posts/kicking-off-the-hummingbot-dashboard-community-project/index.md) where we kicked off the Hummingbot Dashboard project and following progress in the **#dashboard** channel in Discord.
...
[Candles Feed](../v2-strategies/candles/index.md) helps you generate custom OHLCV candles using both historical and live Websocket data, and create real-time custom technical indicators using `pandas_ta`.
...
Following our [Technical Roadmap](../blog/posts/hummingbot-2024-technical-roadmap/index.md), we are expanding our connectors to support to include all order types offered by an exchange. In this release, we integrated the `market_order` and `limit_maker_order` types for [Gate.io](../exchanges/gate-io/index.md).
...
* [#127](https://github.com/hummingbot/gateway/pull/127) Changed default gas setting for [Polygon](../gateway/chains.md) to 400
* [#131](https://github.com/hummingbot/gateway/pull/131) Refactored [TraderJoe](../exchanges/gateway/traderjoe.md) to use SDK V2 **Thanks to [CoinAlpha](https://github.com/coinalpha) for this fix! 🙏**
